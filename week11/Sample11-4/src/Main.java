...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        String[][] employees = {{"홍길동","1111"},{"강민성","1211"},{"김명성","2111"},
                                {"박건준","1441"},{"오유빈","1444"},{"김지민","1456"},
                                {"한정호","1678"},{"박수민","1567"},{"이예린","1789"},
                                {"이순신","1788"}};
    ...
    //    int[][] salarys = {{1,0,0,0,0,0},{4,0,0,0,0,0},};
        ...
        while(i< employees.length) {
            ...
            if(salarys[i][1]>=0 && salarys[i][1] <=1000000) {
                ...
                System.err.println("ERROR : 업무수당은 0~1,000,000");
                System.in.read();
            }
        }
        ...
        System.out.println("*****************************************************************");
        for(i=0;i< employees.length;i++) {
            System.out.printf("%4s %5s %1d %,9d %,9d %,9d %,8d %,9d\n",employees[i][0],employees[i][1],hobong[i],salarys[i][0],salarys[i][1],salarys[i][2],salarys[i][3],salarys[i][4]);
        }
        System.out.println("*****************************************************************");
    }
}